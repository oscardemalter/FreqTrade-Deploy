...
from backend.auth import create_access_token, get_current_user, check_rate_limit, record_attempt, pwd_context, verify_password
...
app = FastAPI(title="Übergestalt", docs_url=None if settings.ENVIRONMENT == "production" else "/docs", redoc_url=None)
...
async def login(request: Request, email: str = Form(...), password: str = Form(...)):
    ...
    if not check_rate_limit(ip):
        return templates.TemplateResponse("login.html", {"request": request, "error": "Trop de tentatives / Too many attempts"}, status_code=429)
    if (email == settings.ADMIN_EMAIL or email == "admin@ubergestalt.local") and verify_password(password, settings.ADMIN_PASSWORD_HASH):
        token = create_access_token({"sub": settings.ADMIN_EMAIL}, timedelta(hours=24))
        resp = RedirectResponse("/dashboard", status_code=303)
        resp.set_cookie("access_token", token, httponly=True,
                        secure=settings.ENVIRONMENT == "production", samesite="strict", max_age=24*3600)
        return resp
    record_attempt(ip)
    return templates.TemplateResponse("login.html", {"request": request, "error": "Mot de passe confidentiel incorrect"}, status_code=401)
...
async def status(user: dict = Depends(get_current_user)):
    ...
    st["engine"] = {"running": native_engine.running, "last_signals": native_engine.last_signals[:10]}
...
async def chat(request: Request, user: dict = Depends(get_current_user)):
    ...
    agent_id = data.get("agent_id", "jarvis")
    if agent_id == "jarvis":
    ...
    return {"agent": agent_id, "reply": agent_response(agent_id, data.get("question", ""), lang)}
...
async def set_password(request: Request, user: dict = Depends(get_current_user)):
    data = await request.json()
    new_pass = data.get("password", "").strip()
    if len(new_pass) < 6:
        return {"success": False, "reason": "Mot de passe trop court (minimum 6 caractères)"}
    settings.update_password(new_pass)
    push_alert("SÉCURITÉ", "Mot de passe d'accès mis à jour avec succès", "info")
...
async def emergency_stop(user: dict = Depends(get_current_user)):
    ...
    push_alert("KILL-SWITCH", "Arrêt d'urgence déclenché — Retour PAPER", "error")
    return res
...
@app.get("/health")
async def health():
    return {"status": "ok", "app": "Übergestalt", "mode": settings.TRADING_MODE}
